...
The database currently exposes 288 physical tables: 160 InnoDB and 128 MyISAM. Import audit/map/snapshot history is retained, but accepted run-scoped recovery tables have been removed after two independent restore-verified backups and a destructive clone trial.
...
- Storefront regression: pagination, SSR page-two loads, history, filter reset, inline retry, URL canonicalization, cached progressive-image hydration and one-shot image fallback pass 16/16 across desktop/mobile Playwright. The production build on `:3001` passed focused 4/4 image checks, category/product/news smoke with zero loaded images stuck blurred, and shared health 15/15.
