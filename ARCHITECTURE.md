...
### PC Builder v5 pricing and export flow

The storefront keeps only component code, product ID and quantity (`1–4`) in drafts/share payloads. `web-admin` remains the price authority: quote loads catalog/Flash Sale prices, evaluates active Build PC promotion requirements against distinct selected SKUs, resolves logical SKU/category-descendant targets, then selects one best promotion per line without stacking it on Flash Sale. The final price is the lower of the two offers. Quote fingerprints include selections, quantities, catalog/config/rule revisions, final prices and the promotion revision; order creation repeats the quote inside its transaction and stores the returned item/promotion snapshot.

Build PC promotion state is isolated in `web_admin_pc_builder_promotions`, `web_admin_pc_builder_promotion_targets` and `web_admin_pc_builder_promotion_requirements`. Only the first table owns schedule/discount/priority state. Target product/category IDs remain logical legacy references; requirements have a physical FK to PC Builder components. The admin route uses the existing `catalog.pc_builder.update` permission, write gate, origin checks, optimistic version and audit trail.

Adding a build to the ordinary cart is intentionally a context boundary: the storefront atomically merges every SKU/quantity using `cartPrice` and emits one cart update/success event, while compatibility fingerprints and Build PC promotions are discarded. Direct PC Builder checkout retains the quote context. Excel/PNG/print are client-side presentation exports and never become trusted order inputs.

### PC Builder v6 direct SKU price

`web_admin_pc_builder_product_prices` is an additive, logical product reference keyed by `product_id`; it does not modify or constrain the legacy price table. Product create/edit persists the optional direct Build PC price in the same transaction as basic catalog data. Runtime accepts only active integer values greater than zero and below the catalog selling price, so out-of-band invalid rows are ignored safely.

Eligibility is recomputed for every quote from active required component configuration and distinct selected SKUs versus `minSelections`. An incomplete configuration uses only catalog/Flash Sale prices and does not evaluate Build PC campaigns. A complete configuration applies a valid direct SKU price as the authoritative final price; only SKUs without a direct price may fall back to campaign pricing. The quote carries `cartSubtotal`, `buildPriceEligible` and `buildPriceRevision`; the fingerprint and transactional order snapshot bind the decision to current selections, quantity, config and prices. Ordinary cart handoff remains outside this context and uses `cartPrice`.
