...
| Homepage featured collection carousel | Implemented and browser-tested | Section 8 renders ten sellable products from collection 896 through the existing bootstrap and remains server-rendered. The homepage-only raw script ported from `index.html` controls every non-hero `.carousel-track` with a one-card buffer, optional clones, three-second auto-slide, mouse/touch threshold gestures, hover pause, indicators and resize recalculation; its Next adapter owns init/destroy cleanup without changing HTML/CSS |
...
| Functional verification | Required compile/test/build matrix passes; full browser/performance rerun pending | Both typecheck/lint/build pipelines pass; 107/107 unit tests and default integration 6 pass/7 fixture- or safety-gated skips. The replacement homepage-carousel suite passes 7 tests with 3 expected project skips across desktop/mobile, including frame-level reset continuity, previous/dot controls and lifecycle cleanup. The prior full 12-worker run remains resource-inconclusive at 44 pass/4 skip/28 fail. Strict health is 13/15 because its legacy collection probes return 404; documented empty-catalog mode is 15/15. |
...
| Web-admin unit tests | 107/107 pass, including homepage featured-collection bounds/cache-key coverage, ApiKey normalization/validation, strict stored-key matching and public category-attribute eligibility/grouping validation |
...
| Local healthcheck | Current strict rerun: 13/15; both configured legacy collection probes return 404, while API products/search/categories/brand/detail/cart, admin auth protection, storefront home/search/brand, and the Section 8 production smoke pass. Historical empty-catalog mode reached 15/15. |
...
| Playwright + axe | Focused category-title/mega-menu rerun: 8 pass/2 expected skips. Replacement homepage-carousel suite: 7 pass/3 expected project skips across desktop/mobile global initialization, timing, frame-level reset continuity, hover, mouse/touch thresholds, previous/dot controls, geometry/request preservation and idempotent clone cleanup. Full 76-test run with 12 workers remains resource-inconclusive at 44 pass/4 skip/28 fail. |
