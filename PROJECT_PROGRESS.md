...
| Web-admin unit tests | 135/135 pass, including canonical page-view path/UUID/source validation, batch aggregation, news-landing scope resolution, public-news mapping, article-category metadata and rich-text uploads |
...
| Default DB integration suite | 16 pass, 7 environment- or safety-gated skips; page-view four-route resolution and rollback-safe exactly-once aggregation pass with news/category/metadata/category-feature regressions; destructive CRUD/import tests remain correctly gated |
...
| Playwright + axe | Full controlled run with 4 workers: 105 pass/17 expected project/device/data skips across all 122 desktop/mobile cases. News coverage now validates landing 2/3/6 binding, normal-flow promo and PCM playlist plus both category/article templates, reusable sidebar behavior, sharing, pagination and edge states. Existing computed-style select and offline TinyMCE smoke remain green. |
