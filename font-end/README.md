...
- Homepage Section 8 reads its configurable featured collection through the existing server-side bootstrap request. The current local configuration is collection `896` / `goi-y-cho-ban`, title `Gợi ý cho bạn`, limited to 10 sellable products; it does not issue a browser-side collection request or retain static product/specification badges. Section 8 remains server-rendered and participates in the homepage-only raw carousel runtime ported from `index.html`: every `.carousel-track` except the hero uses a one-card buffer, optional clones, three-second auto-slide, mouse/touch drag, hover pause, indicators and resize recalculation. A nonvisual Next.js adapter initializes and destroys that runtime without changing section HTML or CSS.
...
The `2026-07-15` working tree passes TypeScript, ESLint `--quiet`, production build, and npm audit with zero known vulnerabilities. The focused category-title/mega-menu run passed 8 tests with 2 expected project/device skips using one worker. The replacement homepage-carousel suite passes 7 tests with 3 expected project skips across desktop/mobile global initialization, timing, frame-level reset continuity, hover, mouse/touch thresholds, previous/dot controls, geometry/request preservation and idempotent clone cleanup. The full 76-test run with 12 workers was resource-inconclusive: 44 pass, 4 skip, and 28 failures dominated by `ERR_INSUFFICIENT_RESOURCES`, navigation timeouts, and cascading selector failures. Rerun with controlled concurrency before classifying product regressions.
