...
- Canonical brand pages at `/brand/[slug]`; Homepage Section 15 uses the bootstrap-provided canonical IDs/slugs, counts and remote PCMarket logos in a responsive two-to-six-column gallery. Missing/broken logos use a text fallback, the initial 366/378px viewport scrolls internally beneath a fade/chevron, and `Xem tất cả` expands all rows in place without an extra browser request. Desktop adds a 24px-radius glass frame inset 16px inside the 16px-radius shell; this decorative inner frame is hidden below 1024px.
