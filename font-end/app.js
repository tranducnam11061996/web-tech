...
async function start() {
  ...
  server.listen(port, host, () => {
    console.log(`[storefront] listening on ${host}:${port} (${dev ? 'development' : 'production'})`);
  });
}
...
function shutdown(signal) {
  ...
  shuttingDown = true;
  console.log(`[storefront] received ${signal}; draining requests`);
  ...
  server.close((error) => {
    ...
    if (error) {
      console.error('[storefront] shutdown failed', error);
      process.exit(1);
      return;
    }
    process.exit(0);
