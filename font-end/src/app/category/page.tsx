    ...
    >
      ...
      >
        ...
        {attr.values.length > 4 && (
          <button
            className="flex items-center gap-3 mt-4 mb-2 ml-1 text-[13px] text-gray-400 hover:text-white transition-colors font-medium"
...
function CategoryContent({ categoryId, params, searchParams }: any) {
  ...
  useEffect(() => {
    ...
    if (activeCatId) {
      ...
      fetch(
        `http://localhost:3000/api/categories/price-bounds?categoryId=${activeCatId}`,
      )
        .then((res) => res.json())
        .then((res) => {
          if (res.success) {
      ...
      fetch(
        `http://localhost:3000/api/categories/attributes?categoryId=${activeCatId}`,
      )
        .then((res) => res.json())
        .then((res) => {
          if (res.success) {
                  ...
                  <div className="p-4 flex flex-col flex-1 relative z-10">
                    ...
                    <div className="flex items-center justify-between mt-auto">
                      <span className="text-white font-extrabold text-[17px] tracking-wide">
                        R {new Intl.NumberFormat("en-ZA").format(product.price)}
                      </span>
