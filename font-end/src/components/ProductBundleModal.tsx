...
export default function ProductBundleModal(props: Props) {
  ...
  return <div className="bundle-list-overlay" onMouseDown={(event) => event.target === event.currentTarget && props.onClose()}>
    <div ref={dialogRef} className="bundle-list-modal" role="dialog" aria-modal="true" aria-labelledby="combo-dialog-title" tabIndex={-1}>
      ...
      <div className="bundle-list-search"><Search size={17} aria-hidden="true"/><label className="sr-only" htmlFor="combo-product-search">Tìm sản phẩm</label><input id="combo-product-search" maxLength={100} value={query} onChange={(event) => { setQuery(event.target.value); setPage(1); }} placeholder="Tìm tên hoặc mã sản phẩm"/></div>
