...
export default function ProductGridCardStatic({ product }: { product: ProductGridCardData }) {
  ...
  return <article className="group relative flex h-full flex-col overflow-hidden rounded-xl border border-[#27272a] bg-gradient-to-b from-[#1a1a1d] to-[#111113] shadow-[0_4px_12px_rgba(0,0,0,0.2)] transition-all duration-300 hover:-translate-y-1.5 hover:border-[#3f3f46] hover:shadow-[0_8px_30px_rgba(0,0,0,0.6)]">
    ...
    <Link href={`/${slug}`} className="flex h-full flex-1 flex-col">
      <div className="relative flex aspect-[4/3] w-full items-center justify-center bg-[#151518]">
        {product.thumbnail ? <Image src={product.thumbnail} alt={product.name} fill sizes="(max-width: 640px) 50vw, (max-width: 1024px) 33vw, 20vw" className="object-cover" /> : <span className="text-xs font-bold text-zinc-600">TrucTiepGAME</span>}
