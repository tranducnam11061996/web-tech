...
function localMediaUrl(relativePath: string) {
  return `/api/media/news/${relativePath.split('/').map(encodeURIComponent).join('/')}`;
}
...
export async function POST(request: Request) {
  try {
    ...
    if (file.size > MAX_FILE_SIZE) throw new AdminApiError(400, 'BAD_REQUEST', 'Dung lượng ảnh tối đa là 10MB');
    ...
    const folderPath = path.resolve(/*turbopackIgnore: true*/ mediaRoot, 'news', dayFolder);
    ...
    const baseName = sanitizeName(file.name, `article-${Date.now()}`);
    ...
    const relativePath = `${dayFolder}/${fileName}`;
    return ok({ fileName, relativePath, url: localMediaUrl(relativePath) }, 'Tải ảnh thành công');
  } catch (error) {
    return fail(error);
  }
}
