...
export async function POST(request: Request) {
  try {
    ...
    const relativeFolder = `category/${dayFolder}`;
    const folderPath = path.resolve(/*turbopackIgnore: true*/ mediaRoot, 'category', dayFolder);
    ...
    const baseName = sanitizeName(file.name, `category-${Date.now()}`);
    ...
    return ok({ field, fileName, relativePath, url: mediaUrl(relativePath) }, 'Tải ảnh thành công');
  } catch (error) {
    return fail(error);
  }
}
