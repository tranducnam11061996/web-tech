...
import { reorderProductComboSets } from '@/lib/comboSets';
...
export async function PATCH(request: Request, context: ProductComboRouteContext) {
  try {
    await requireAdminWrite(request);
    const { id } = await context.params;
    const body = await request.json().catch(() => ({}));
    const setIds = Array.isArray(body?.setIds) ? body.setIds.map(toInt) : [];
    await reorderProductComboSets(toInt(id), setIds);
    return ok({ items: await listProductComboSets(toInt(id)) }, 'Đã cập nhật thứ tự combo set');
  } catch (error) {
    return fail(error);
  }
}
