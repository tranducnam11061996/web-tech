...
export function CollectionProductFilter({
  collectionId,
  collectionName,
  initialSearch = '',
}: {
  ...
  const submit = (event: FormEvent) => {
    ...
    params.set('id', String(collectionId));
    params.set('page', '1');
    ...
    <div className="flex flex-col gap-4 mb-4">
      <div className="flex justify-between items-center bg-[#0a0a0f]/90 backdrop-blur-md p-3 rounded-lg border border-gray-800/50 shadow-sm z-20">
        <h1 className="text-xl font-bold text-transparent bg-clip-text bg-gradient-to-r from-red-500 to-blue-500 flex items-center gap-3">
          ...
          <span className="max-w-[360px] truncate text-sm font-medium normal-case text-gray-300">#{collectionId} - {collectionName}</span>
        </h1>
      ...
      <AddProductModal collectionId={collectionId} isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} />
    </div>
  );
}
