...
export function ProductPromotionManager({ editPromotionId, standalone = false }: { editPromotionId?: number; standalone?: boolean }) {
  ...
  const startEdit = async (id: number) => {
    ...
    try {
      ...
      setForm({
        ...
        displayOrder: String(value.displayOrder ?? 0), startsAt: value.startsAt || '', endsAt: value.endsAt || '',
  ...
  const submit = async (event: React.FormEvent) => {
    ...
    const displayOrderText = form.displayOrder.trim();
    if (!form.displayText.trim()) errors.displayText = 'Nhập nội dung hiển thị.';
    if (displayOrderText && (!/^\d+$/.test(displayOrderText) || Number(displayOrderText) > 65_535)) {
      errors.displayOrder = 'Thứ tự ưu tiên phải là số nguyên từ 0 đến 65.535.';
    }
    ...
    try {
      const response = await fetch(editingId ? `/api/admin/product-promotions/${editingId}` : '/api/admin/product-promotions', {
        ...
        body: JSON.stringify({
          ...
          displayOrder: displayOrderText ? Number.parseInt(displayOrderText, 10) : 0,
  ...
  return <section className="space-y-4">
    ...
    {formOpen ? <div className={standalone ? 'mx-auto max-w-6xl py-2' : 'fixed inset-0 z-50 overflow-y-auto bg-black/75 px-4 pb-4 backdrop-blur-sm'} onMouseDown={standalone ? undefined : (event) => { if (event.target === event.currentTarget) closeForm(); }}><form ref={dialogRef} role={standalone ? undefined : 'dialog'} aria-modal={standalone ? undefined : true} aria-labelledby="promotion-form-title" aria-describedby="promotion-form-description" noValidate onSubmit={submit} className={`mx-auto max-w-6xl rounded-2xl border border-gray-700 bg-[#0d0d12] shadow-2xl ${standalone ? 'my-4' : 'mb-4 mt-20'}`}>
      ...
      <div className="space-y-6 p-5">
        ...
        <fieldset className="rounded-xl border border-gray-800 p-4"><legend className="px-2 text-sm font-semibold text-white">1. Nội dung hiển thị</legend><div className="grid gap-4 lg:grid-cols-[minmax(0,1fr)_220px]">
          ...
          <label className="block text-sm font-medium text-gray-200">Đường dẫn “Xem chi tiết”<input value={form.detailUrl} aria-invalid={Boolean(fieldErrors.detailUrl)} aria-describedby={fieldErrors.detailUrl ? 'promotion-url-error' : 'promotion-url-help'} onChange={(event) => setForm((current) => ({ ...current, detailUrl: event.target.value }))} placeholder="/khuyen-mai/... hoặc https://..." className="mt-2 min-h-11 w-full rounded-lg border border-gray-700 bg-gray-950 px-3 text-white outline-none focus:border-red-500" /><span id="promotion-url-help" className="mt-1 block text-xs text-gray-500">Không bắt buộc. Nếu nhập, chỉ chấp nhận đường dẫn nội bộ hoặc HTTPS.</span>{fieldErrors.detailUrl ? <span id="promotion-url-error" className="mt-1 block text-xs text-red-300">{fieldErrors.detailUrl}</span> : null}</label>
          <label className="block text-sm font-medium text-gray-200">Thứ tự ưu tiên<input type="text" inputMode="numeric" pattern="[0-9]*" value={form.displayOrder} aria-invalid={Boolean(fieldErrors.displayOrder)} aria-describedby={fieldErrors.displayOrder ? 'promotion-order-error' : 'promotion-order-help'} onChange={(event) => setForm((current) => ({ ...current, displayOrder: event.target.value }))} className="mt-2 min-h-11 w-full rounded-lg border border-gray-700 bg-gray-950 px-3 tabular-nums text-white outline-none focus:border-red-500" /><span id="promotion-order-help" className="mt-1 block text-xs text-gray-500">Số nguyên từ 0 đến 65.535; để trống sẽ dùng 0. Số nhỏ hơn xuất hiện trước.</span>{fieldErrors.displayOrder ? <span id="promotion-order-error" className="mt-1 block text-xs text-red-300">{fieldErrors.displayOrder}</span> : null}</label>
          ...
          <div className="rounded-xl border border-blue-900/40 bg-blue-950/15 p-3 lg:col-span-2"><p className="text-xs font-semibold uppercase tracking-wide text-blue-300">Xem trước storefront</p><div className="mt-3 flex items-start gap-3"><span className="grid h-7 w-7 flex-none place-items-center rounded-full bg-blue-600 text-xs font-bold text-white">01</span><p className="text-sm leading-6 text-gray-200">{form.displayText || 'Nội dung khuyến mãi sẽ xuất hiện tại đây.'}{form.detailUrl.trim() ? <> <a href={form.detailUrl.trim()} onClick={(event) => event.preventDefault()} className="inline-flex min-h-10 items-center font-semibold text-sky-400">Xem chi tiết</a></> : null}</p></div></div>
        </div></fieldset>
