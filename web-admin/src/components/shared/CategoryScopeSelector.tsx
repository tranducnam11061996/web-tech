...
export type VisibleCategoryScopeNode = CategoryScopeItem & { children: VisibleCategoryScopeNode[] };
...
export function filterCategoryTreeForSearch(nodes: VisibleCategoryScopeNode[], query: string): VisibleCategoryScopeNode[] {
  return nodes.flatMap((node) => {
    const children = filterCategoryTreeForSearch(node.children, query);
    const matches = normalizeCategorySearch(`${node.name} ${node.id}`).includes(query);
    return matches || children.length > 0 ? [{ ...node, children }] : [];
  });
}

function collectExpandableNodeIds(nodes: VisibleCategoryScopeNode[], result = new Set<number>()) {
...
export function descendantsOf(id: number, childrenByParent: Map<number, number[]>) {
  const result: number[] = [];
...
export function nextCategoryScopeSelection(
  id: number,
  selectedIds: number[],
  categoryById: Map<number, CategoryScopeItem>,
...
export function CategoryScopeSelector({
  ...
  const selectedTitleId = `${generatedId}-selected-title`;
  const treeTitleId = `${generatedId}-tree-title`;
  ...
  const { roots, expandableIds, selectedAncestorIds } = useMemo(() => {
    ...
    const activeNodes = new Map(activeCategories.map((category) => [category.id, { ...category, children: [] as VisibleCategoryScopeNode[] }]));
    const nextRoots: VisibleCategoryScopeNode[] = [];
    ...
    const sortTree = (nodes: VisibleCategoryScopeNode[]) => {
      nodes.sort((left, right) => left.name.localeCompare(right.name, 'vi'));
      nodes.forEach((node) => sortTree(node.children));
    };
    sortTree(nextRoots);
    const nextExpandableIds = new Set<number>();
    const walk = (nodes: VisibleCategoryScopeNode[], visited = new Set<number>()) => nodes.forEach((node) => {
  ...
  const visibleRoots = useMemo(
    () => normalizedSearchQuery ? filterCategoryTreeForSearch(roots, normalizedSearchQuery) : roots,
  ...
  const toggleCheck = (id: number) => {
    onChange(nextCategoryScopeSelection(id, selectedIds, categoryById, childrenByParent));
  };
  ...
  const renderTree = (nodes: VisibleCategoryScopeNode[], visited = new Set<number>()): ReactNode => nodes.map((node) => {
    if (visited.has(node.id)) return null;
    visited.add(node.id);
    const hasChildren = node.children.length > 0;
    const isOpen = openIds.has(node.id);
    const isChecked = selectedIds.includes(node.id);
    const childrenId = `${generatedId}-children-${node.id}`;
    return <div key={node.id} className="flex flex-col">
      <div className="flex min-h-10 items-center gap-2 rounded-md px-1 transition-colors hover:bg-gray-800/40">
        {hasChildren ? <button type="button" onClick={() => toggleOpen(node.id)} aria-label={`${isOpen ? 'Thu gọn' : 'Mở rộng'} ${node.name}`} aria-expanded={isOpen} aria-controls={childrenId} className="grid h-10 w-10 place-items-center text-gray-500 hover:text-gray-200 focus-visible:outline-2 focus-visible:outline-blue-400">{isOpen ? <MinusSquare aria-hidden="true" className="h-4 w-4" /> : <PlusSquare aria-hidden="true" className="h-4 w-4" />}</button> : <span aria-hidden="true" className="h-10 w-10" />}
        <label className="flex min-h-10 flex-1 cursor-pointer items-center gap-2 rounded-sm text-left text-sm text-gray-300 focus-within:outline-2 focus-within:outline-blue-400">
          <input type="checkbox" checked={isChecked} onChange={() => toggleCheck(node.id)} className="sr-only" />
      ...
      {isOpen && hasChildren ? <div id={childrenId} className="ml-10 border-l border-gray-800 pl-2">{renderTree(node.children, new Set(visited))}</div> : null}
    </div>;
  });

  const selectedItemList = selectedItems.length > 0 ? selectedItems.map((item) => <div key={item.id} className={`flex items-center gap-2 rounded-lg border px-3 py-2 text-sm ${item.missing ? 'border-amber-700/40 bg-amber-950/20 text-amber-200' : 'border-blue-800/40 bg-blue-950/20 text-blue-100'}`}><span className="min-w-0 flex-1 truncate">{item.name}</span><button type="button" onClick={() => toggleCheck(item.id)} aria-label={`Bỏ ${item.name}`} className="grid h-10 w-10 flex-none place-items-center rounded-md text-gray-400 hover:bg-gray-800 hover:text-white focus-visible:outline-2 focus-visible:outline-blue-400"><X aria-hidden="true" className="h-4 w-4" /></button></div>) : null;
  const searchControl = searchable ? <div className="relative mt-3"><label htmlFor={searchId} className="sr-only">Tìm danh mục áp dụng</label><Search aria-hidden="true" className="absolute left-3 top-3.5 h-4 w-4 text-gray-500" /><input ref={searchInputRef} id={searchId} type="search" value={searchQuery} onChange={(event) => setSearchQuery(event.target.value)} placeholder="Tìm theo tên hoặc ID danh mục" className="min-h-11 w-full rounded-lg border border-gray-700 bg-gray-950 pl-10 pr-11 text-sm text-white outline-none focus:border-blue-500" />{searchQuery ? <button type="button" onClick={() => { setSearchQuery(''); searchInputRef.current?.focus(); }} aria-label="Xóa từ khóa tìm danh mục" className="absolute right-1 top-0.5 grid h-10 w-10 place-items-center rounded-md text-gray-500 hover:bg-gray-800 hover:text-white focus-visible:outline-2 focus-visible:outline-blue-400"><X aria-hidden="true" className="h-4 w-4" /></button> : null}</div> : null;
  const treeControls = <><div className="flex items-center justify-between gap-3"><h3 id={treeTitleId} className="text-sm font-semibold text-white">{treeTitle}</h3><button type="button" onClick={toggleAll} disabled={expandableIds.size === 0} className="min-h-10 rounded-lg border border-gray-700 px-3 text-xs font-medium text-gray-300 hover:border-blue-600 hover:text-blue-300 focus-visible:outline-2 focus-visible:outline-blue-400 disabled:opacity-50">{isAllExpanded ? 'Thu gọn' : 'Mở tất cả'}</button></div>{searchControl}<div className="mt-3 max-h-80 overflow-y-auto pr-1">{visibleRoots.length > 0 ? renderTree(visibleRoots) : <p className="py-8 text-center text-sm text-gray-500">{normalizedSearchQuery ? 'Không tìm thấy danh mục phù hợp.' : 'Không có danh mục đang hoạt động.'}</p>}</div></>;

  return <div className="grid gap-4 lg:grid-cols-[minmax(0,0.85fr)_minmax(0,1.15fr)]">
    <section aria-labelledby={selectedTitleId} className="rounded-xl border border-gray-800 bg-gray-950/60 p-4">
      <h3 id={selectedTitleId} className="text-sm font-semibold text-white">{selectedTitle}</h3>
      <p className="mt-1 text-xs leading-5 text-gray-500">{helpText}</p>
      <div className="mt-3 max-h-80 space-y-2 overflow-y-auto pr-1">
        {selectedItemList || <p className="rounded-lg border border-dashed border-gray-800 px-3 py-6 text-center text-sm text-gray-500">{emptyText}</p>}
      </div>
    </section>
    <section aria-labelledby={treeTitleId} className="rounded-xl border border-gray-800 bg-gray-950/60 p-4">
      {treeControls}
    </section>
  </div>;
}
