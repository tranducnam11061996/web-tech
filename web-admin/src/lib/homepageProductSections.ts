...
type ProductRow = RowDataPacket & {
  categoryId: number; id: number; storeSKU: string | null; proName: string | null; proThum: string | null;
  price: number | null; market_price: number | null; slug: string | null; brandName: string | null;
};

export function parseHomepageCategoryIds(value: string | null) {
  if (!value) return [];
  const ids: number[] = [];
...
export function parseHomepageProductLimit(value: string | null) {
  const productLimit = Number(value || 8);
  return Number.isFinite(productLimit) ? Math.min(24, Math.max(1, Math.trunc(productLimit))) : 8;
}

function normalizeSlug(value: unknown) { return String(value || '').replace(/^\/+/, ''); }

export async function loadHomepageProductSections(categoryIds: number[], productLimit: number) {
  if (categoryIds.length === 0) return { sections: [], meta: { generatedAt: new Date().toISOString() } };
  ...
  for (const product of productRows) {
    const products = productsByCategoryId.get(Number(product.categoryId)) || [];
    products.push(product);
    productsByCategoryId.set(Number(product.categoryId), products);
  }
  return {
