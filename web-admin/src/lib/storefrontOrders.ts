...
import { enqueueOrderEmail } from '@/lib/orderInfrastructure';
import type { EmailCustomer, EmailDelivery, EmailOrderItem, EmailOrderTotals } from '@/lib/email';
import { transitionFlashSaleAllocations } from '@/lib/flashSales';
...
export async function listAdminStorefrontOrders(params: URLSearchParams) {
  ...
  const orderType = String(params.get('orderType') || ''); if (orderType === 'combo' || orderType === 'standard' || orderType === 'pc_builder') { where.push('m.order_type = ?'); values.push(orderType); }
  const [rows] = await pool.query<RowDataPacket[]>(`SELECT o.id,o.product_title,o.total_value,o.status,o.create_time,m.customer_name,m.customer_phone,m.payment_status,m.shipping_status,m.assigned_admin_name,m.order_type,m.combo_set_id,m.combo_anchor_product_id,m.pc_build_id,m.assembly_required,m.pc_builder_revision,r.code_snapshot,r.discount_amount,(SELECT COUNT(*) FROM build_buy_item i WHERE i.order_id=o.id) item_count FROM build_buy o LEFT JOIN web_admin_storefront_order_meta m ON m.order_id=o.id LEFT JOIN web_admin_voucher_redemptions r ON r.order_id=o.id ${where.length ? `WHERE ${where.join(' AND ')}` : ''} ORDER BY o.id DESC LIMIT ?`, [...values, limit + 1]);
  const hasMore = rows.length > limit; const items = rows.slice(0, limit).map((row) => ({ id:Number(row.id), productTitle:String(row.product_title||''), totalValue:Number(row.total_value||0), status:Number(row.status), statusLabel:statusLabel(Number(row.status)), createTime:Number(row.create_time||0), customerName:String(row.customer_name||''), customerPhone:String(row.customer_phone||''), paymentStatus:String(row.payment_status||'unpaid'), shippingStatus:String(row.shipping_status||'pending'), assignedAdminName:String(row.assigned_admin_name||''), orderType:String(row.order_type||'standard'), comboSetId:row.combo_set_id?Number(row.combo_set_id):null, comboAnchorProductId:row.combo_anchor_product_id?Number(row.combo_anchor_product_id):null, pcBuildId:row.pc_build_id?Number(row.pc_build_id):null, assemblyRequired:Boolean(row.assembly_required), pcBuilderRevision:String(row.pc_builder_revision||''), voucherCode:row.code_snapshot||null, voucherDiscount:Number(row.discount_amount||0), itemCount:Number(row.item_count||0) }));
  return { items, nextCursor: hasMore ? items.at(-1)?.id || null : null, hasMore };
}

export async function getAdminStorefrontOrder(id: number) {
  const [headers] = await pool.query<RowDataPacket[]>(`SELECT o.*,m.customer_name,m.customer_phone,m.customer_email,m.payment_method,m.delivery_method,m.payment_status,m.shipping_status,m.assigned_admin_user_id,m.assigned_admin_name,m.order_type,m.combo_set_id,m.combo_anchor_product_id,m.pc_build_id,m.assembly_required,m.pc_builder_revision,r.code_snapshot,r.title_snapshot,r.discount_amount,r.eligible_subtotal FROM build_buy o LEFT JOIN web_admin_storefront_order_meta m ON m.order_id=o.id LEFT JOIN web_admin_voucher_redemptions r ON r.order_id=o.id WHERE o.id=? LIMIT 1`, [id]);
  ...
  return { id:Number(order.id), title:String(order.product_title||''), totalValue:Number(order.total_value||0), status:Number(order.status), statusLabel:statusLabel(Number(order.status)), createTime:Number(order.create_time||0), orderType:String(order.order_type||'standard'), combo:config.combo||null, pcBuilder:config.pcBuilder||null, customer:buyer.customer||{}, receiver:buyer.receiver||{}, delivery:buyer.delivery||{}, invoice:buyer.invoice||{}, note:config.note||'', totals:config.totals||{}, voucher:order.code_snapshot?{code:order.code_snapshot,title:order.title_snapshot,discount:Number(order.discount_amount||0),eligibleSubtotal:Number(order.eligible_subtotal||0)}:null, meta:{paymentMethod:order.payment_method||buyer.paymentMethod||'',deliveryMethod:order.delivery_method||buyer.delivery?.method||'',paymentStatus:order.payment_status||'unpaid',shippingStatus:order.shipping_status||'pending',assignedAdminUserId:order.assigned_admin_user_id||null,assignedAdminName:order.assigned_admin_name||'',comboSetId:order.combo_set_id||null,comboAnchorProductId:order.combo_anchor_product_id||null,pcBuildId:order.pc_build_id?Number(order.pc_build_id):null,assemblyRequired:Boolean(order.assembly_required),pcBuilderRevision:String(order.pc_builder_revision||'')}, items:items.map((x)=>({productId:Number(x.product_id),title:String(x.title||''),sku:x.storeSKU||'',slug:x.slug||'',thumbnail:x.proThum||'',price:Number(x.product_price||0),quantity:Number(x.quantity||0),lineTotal:Number(x.product_price||0)*Number(x.quantity||0)})), events:events.map((x)=>({id:Number(x.id),type:x.event_type,from:x.from_value,to:x.to_value,note:x.note,actor:x.actor_name,createdAt:x.created_at})), admins:admins.map((x)=>({id:Number(x.id),name:String(x.name),email:String(x.email)})) };
}

async function enqueueCompletedOrderEmail(connection: PoolConnection, id: number) {
  const [orders] = await connection.query<RowDataPacket[]>(`SELECT o.id,o.total_value,o.buyer_info,o.config,m.customer_email,m.order_type
    FROM build_buy o LEFT JOIN web_admin_storefront_order_meta m ON m.order_id=o.id WHERE o.id=? LIMIT 1`, [id]);
  const order = orders[0]; if (!order) return;
  const buyer = snapshot(order.buyer_info); const config = snapshot(order.config);
  const [itemRows] = await connection.query<RowDataPacket[]>('SELECT product_id,title,product_price,quantity FROM build_buy_item WHERE order_id=? ORDER BY id', [id]);
  const customerSource = snapshot(JSON.stringify(buyer.customer || {}));
  const deliverySource = snapshot(JSON.stringify(buyer.delivery || {}));
  const customer: EmailCustomer = { name:String(customerSource.name||''),phone:String(customerSource.phone||''),email:String(customerSource.email||order.customer_email||'') };
  const delivery: EmailDelivery = { method:deliverySource.method==='shipping'?'shipping':'pickup',province:String(deliverySource.province||''),ward:String(deliverySource.ward||''),address:String(deliverySource.address||''),note:String(deliverySource.note||'') };
  const items: EmailOrderItem[] = itemRows.map((row)=>({productId:Number(row.product_id),title:String(row.title||''),quantity:Number(row.quantity||0),product_price:Number(row.product_price||0)}));
  const totalConfig = snapshot(JSON.stringify(config.totals || {}));
  const totals: EmailOrderTotals = { subtotal:Number(totalConfig.subtotal||order.total_value||0),total:Number(totalConfig.total||order.total_value||0),itemCount:Number(totalConfig.itemCount||items.reduce((sum,item)=>sum+item.quantity,0)) };
  await enqueueOrderEmail(connection,{to:customer.email,orderId:id,items,totals,customer,delivery,paymentMethod:String(buyer.paymentMethod||'bank_transfer'),orderType:order.order_type==='combo'?'combo':order.order_type==='pc_builder'?'pc_builder':'standard',eventType:'completed',comboDiscount:Number(config.combo?.discount||0)});
}
...
export async function patchAdminStorefrontOrder(connection: PoolConnection, id:number, payload:any, actor:{id:number;name:string}) {
  ...
  if(payload.orderStatus!==undefined){const next=Number(payload.orderStatus);const current=Number(order.status);if(!ORDER_STATUSES.has(next))throw new AdminApiError(400,'BAD_REQUEST','Trạng thái đơn hàng không hợp lệ.');if([3,4,5].includes(current)&&next!==current)throw new AdminApiError(409,'CONFLICT','Đơn hàng đã kết thúc.');if(next!==current&&next===3)await transitionFlashSaleAllocations(connection,id,'consumed');if(next!==current&&[4,5].includes(next)){await transitionFlashSaleAllocations(connection,id,'released');await releaseVoucherForOrder(connection,id);}if(next!==current){await connection.query('UPDATE build_buy SET status=?,last_update=?,last_update_by=? WHERE id=?',[next,Math.floor(Date.now()/1000),actor.name,id]);await event('order_status',current,next);if(next===3)await enqueueCompletedOrderEmail(connection,id);}}
