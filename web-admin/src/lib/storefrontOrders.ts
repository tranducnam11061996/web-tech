...
export async function createOrderMeta(db: PoolConnection | typeof pool, orderId: number, buyerInfo: Record<string, any>, addEvent = true) {
  ...
  await db.query(`INSERT IGNORE INTO web_admin_storefront_order_meta (order_id,customer_name,customer_phone,customer_email,payment_method,delivery_method) VALUES (?,?,?,?,?,?)`, [orderId, String(customer.name || '').slice(0, 150), String(customer.phone || '').slice(0, 32), String(customer.email || '').slice(0, 255), String(buyerInfo.paymentMethod || ''), String(delivery.method || '')]);
...
export async function listAdminStorefrontOrders(params: URLSearchParams) {
  ...
  const [rows] = await pool.query<RowDataPacket[]>(`SELECT o.id,o.product_title,o.total_value,o.status,o.create_time,m.customer_name,m.customer_phone,m.payment_status,m.shipping_status,m.assigned_admin_name,r.code_snapshot,r.discount_amount,(SELECT COUNT(*) FROM build_buy_item i WHERE i.order_id=o.id) item_count FROM build_buy o LEFT JOIN web_admin_storefront_order_meta m ON m.order_id=o.id LEFT JOIN web_admin_voucher_redemptions r ON r.order_id=o.id ${where.length ? `WHERE ${where.join(' AND ')}` : ''} ORDER BY o.id DESC LIMIT ?`, [...values, limit + 1]);
  const hasMore = rows.length > limit; const items = rows.slice(0, limit).map((row) => ({ id:Number(row.id), productTitle:String(row.product_title||''), totalValue:Number(row.total_value||0), status:Number(row.status), statusLabel:statusLabel(Number(row.status)), createTime:Number(row.create_time||0), customerName:String(row.customer_name||''), customerPhone:String(row.customer_phone||''), paymentStatus:String(row.payment_status||'unpaid'), shippingStatus:String(row.shipping_status||'pending'), assignedAdminName:String(row.assigned_admin_name||''), voucherCode:row.code_snapshot||null, voucherDiscount:Number(row.discount_amount||0), itemCount:Number(row.item_count||0) }));
  return { items, nextCursor: hasMore ? items.at(-1)?.id || null : null, hasMore };
}

export async function getAdminStorefrontOrder(id: number) {
  const [headers] = await pool.query<RowDataPacket[]>(`SELECT o.*,m.customer_name,m.customer_phone,m.customer_email,m.payment_method,m.delivery_method,m.payment_status,m.shipping_status,m.assigned_admin_user_id,m.assigned_admin_name,r.code_snapshot,r.title_snapshot,r.discount_amount,r.eligible_subtotal FROM build_buy o LEFT JOIN web_admin_storefront_order_meta m ON m.order_id=o.id LEFT JOIN web_admin_voucher_redemptions r ON r.order_id=o.id WHERE o.id=? LIMIT 1`, [id]);
