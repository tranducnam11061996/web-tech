...
test('brand sync atomically merges aliases, converts UTF-8 tables and completely rolls back', { skip: !enabled, timeout: 180_000 }, async () => {
  ...
  const [productCounts] = await db.query<any[]>('SELECT CASE brandId WHEN 34 THEN 25 WHEN 57 THEN 31 ELSE brandId END AS id,COUNT(*) AS total FROM idv_sell_product_store WHERE brandId>0 GROUP BY id');
  ...
  assert.equal(await count(db, 'idv_brand'), 89);
  assert.equal(await count(db, 'idv_brand_info'), 89);
  assert.equal(await count(db, 'idv_brand_category'), 1_209);
  assert.equal(await count(db, 'product_data_search'), 4_712);
  assert.equal(await count(db, 'idv_brand', 'WHERE id IN (34,57)'), 0);
  assert.equal(await count(db, 'idv_sell_product_store', 'WHERE brandId IN (34,57)'), 0);
  assert.equal(await count(db, 'idv_sell_product_price', 'WHERE brandId IN (34,57)'), 0);
  assert.equal(await count(db, 'idv_product_category', 'WHERE brandId IN (34,57)'), 0);
  ...
  assert.equal(await count(db, 'web_admin_import_records', `WHERE run_id=${Number(applied.runId)} AND entity='brand'`), 91);
  const [maps] = await db.query<any[]>("SELECT source_id,target_id FROM web_admin_import_entity_map WHERE source='pcmarket' AND entity='brand' AND source_id IN ('34','57') ORDER BY source_id");
  assert.deepEqual(maps.map((row) => [String(row.source_id), String(row.target_id)]), [['34', '25'], ['57', '31']]);
